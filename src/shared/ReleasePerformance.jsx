...
const ReleasePerformance = ({open, onClose, classCode, studentsIds, refresh}) => {
    ...
    const getData = async () => {
        ...
        for (const i in studentsIds) {
            ...
            const studentPerformance = (await classesRef.child(classCode).child('alunos').child(id).child('desempenho').once('value')).val();
            ...
            try {
                ...
                for (const i in studentsIds) {
                    if (Object.hasOwnProperty.call(studentsIds, i)) {
                        const id = studentsIds[i];
                        await classesRef.child(classCode).child('alunos').child(id).child('desempenho').set(localGrades);
                        await classesRef.child(classCode).child('alunos').child(id).child('notas/Desempenho').set(sum);
                    }
                }
                ...
                <DialogContent>
                    ...
                    <div className={classes.flex}>
                        ...
                        <div>
                            ...
                            <label>Somatório: <label style={{color: sum > performanceGradesSum ? "#FF0000" : ""}}>{sum}</label>/{performanceGradesSum}</label>
                        </div>
